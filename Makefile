...
fixtures:  ## rebuild the synthetic reference documents; deterministic, no credentials
	uv run --group samples python samples/generate_synthetic_samples.py samples
...
test: fixtures  ## unit and contract tests, fake profile, no network, no credentials
...
coverage: fixtures  ## the suite plus the core coverage gate. `test` is the same run, without the gate
	DIS_PROFILE=fake uv run pytest tests/unit tests/contract -q \
...
eval: fixtures  ## regression against the frozen baseline of PROFILE (fake by default); non-zero exit on regression
