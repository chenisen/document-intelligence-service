"""Synchronous HTTP boundary. One endpoint: receive a document, return its analysis."""
...
# SpooledTemporaryFile(max_size=0) stays in memory. The framework's 1 MB default
# would persist accepted uploads before the endpoint even saw them (RNF-05).
...
def read_within_limit(stream: BinaryIO, limit: int = MAX_FILE_BYTES) -> bytes:
    """Read the upload, giving up as soon as it passes the limit.

    Reading in chunks means an oversized file never gets fully materialised by us. The first line of
    defence is upstream anyway: API Gateway and the synchronous Lambda invocation both refuse a body
    larger than theirs before this code runs.
    """
    chunks: list[bytes] = []
    total = 0
...
def handle_validation_error(request: Request, error: Exception) -> JSONResponse:
    """FastAPI answers 422 in its own shape by default; the contract has one error shape only."""
