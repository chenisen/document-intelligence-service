"""Spec 0006: independent scoring, explicit synthetic provenance and safe artifacts."""
...
def test_evaluation_counts_classification_and_refusals():
    ...
    cases = json.loads((ROOT / "samples/gabarito.json").read_text())
    pending = iter(cases)
    seen = []
    ...
    # Counted from the answer key, not written as a number: curating the reference set is a normal
    # thing to do, and a magic count would turn it into a test failure instead of a smaller set.
    ...
    # Sem denominador a acurácia é `None`, e não 1.0: taxa sobre zero caso não é acerto perfeito.
    # A chave existe de qualquer forma, porque a forma do relatório não pode depender dos dados.
