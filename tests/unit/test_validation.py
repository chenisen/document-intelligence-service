"""RF-06, RF-07 and RF-11: integrity rules over values already in the contract vocabulary."""
...
# Built here rather than loaded from configuration: what is under test is the rule, not the
# catalogue that happens to be shipped. `test_catalog.py` covers the shipped one.
...
def test_values_outside_the_contract_vocabulary_fail_instead_of_being_repaired(
    ...
    """A prompt that stops honouring the output schema has to reach a person.

    Translating the model's answer here would hide exactly that, so the service does not do it:
    the vocabulary is declared in the prompt, which is Bedrock configuration, and a value outside
    it fails the rule, lowers the confidence and puts the case in review.
    """
