"""Only operational metadata may cross the logging and metrics boundaries."""
...
def test_service_has_no_document_writing_adapter():
    ...
    for path in (ROOT / "src/adapters").rglob("*.py"):
        for node in ast.walk(ast.parse(path.read_text())):
            # Image.save serializes bytes to BytesIO for normalisation, not a document store.
            if (
...
def test_a_revoked_norm_is_never_cited() -> None:
    """A norma revogada existe no corpus e não pode aparecer numa resposta.

    Num índice real isso acontece na ingestão, porque o documento sai do índice. Aqui é um filtro,
    para que a regra continue provável sem índice provisionado.
    """
