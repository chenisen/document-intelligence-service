...
def test_pdf_with_text_layer_chooses_direct_text() -> None:
    chars, texts = read_pdf_text((SAMPLES / "atestado_01_pdf_nativo.pdf").read_bytes())
...
def test_scanned_pdf_chooses_ocr() -> None:
    chars, _ = read_pdf_text((SAMPLES / "atestado_02_pdf_digitalizado.pdf").read_bytes())
...
def test_rasterises_a_multipage_pdf_page_by_page() -> None:
    pages = render_document_pages((SAMPLES / "atestado_08_duas_paginas.pdf").read_bytes())
