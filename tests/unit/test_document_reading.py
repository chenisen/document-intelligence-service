"""Regressões de qualidade local, liberação de recurso e limite de página no conjunto."""
...
@pytest.mark.parametrize("operation", [documents.read_pdf, documents.rasterize])
def test_releases_pdf_resources_after_reading(operation, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    args = (content, 1) if operation is documents.rasterize else (content,)
...
def test_the_reference_index_keeps_each_page_text_separate() -> None:
    """Texto de uma página não pode vazar para a outra.

    O documento de duas páginas tem um anexo só na segunda. Se o índice juntasse as páginas, a
    evidência de um campo poderia apontar para a página errada e a verificação passaria mesmo assim.
    """
    conteudo = (SAMPLES / "atestado_08_duas_paginas.pdf").read_bytes()
    paginas = documents.pages_of(conteudo)
...
def test_evidence_names_the_page_where_the_text_was_found() -> None:
    """A evidência carrega a página real, não a primeira por conveniência."""
    conteudo = (SAMPLES / "atestado_08_duas_paginas.pdf").read_bytes()
    paginas = documents.pages_of(conteudo)
